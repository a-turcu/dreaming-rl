...
while True:  # Run until solved
    ...
    for timestep in range(1, max_steps_per_episode):
        ...
        if frame_count % update_after_actions == 0 and len(exp_replay.rewards) > batch_size:

            # Get batch of experiences from replay buffer
            states_sample, actions_sample, rewards_sample, states_next_sample, dones_sample = exp_replay.sample_experiences()
            ...
            masks = tf.one_hot(actions_sample, num_actions)

            with tf.GradientTape() as tape:
                # Train the model on the states and updated Q-values
                q_values = model(states_sample)
        ...
        if frame_count % update_target_network == 0:
            ...
            np.save(path +"graph_data/rewards.npy", rewards)

            episodes = np.load(path + "graph_data/epsiodes.npy")
            episodes = np.append(episodes, episode_count)
            np.save(path + "graph_data/epsiodes.npy", episodes)
            #env.render()
    ...
    if episode_count % 10000 == 0:
        model.save(f"{path}models/model{episode_count}.h5")

model.save(f"{path}models/model_done.h5")
